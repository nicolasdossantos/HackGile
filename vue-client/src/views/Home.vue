<template>
  <v-layout fill-height class="light-green lighten-4">
    ...
    <v-snackbar v-model="memberAddSnack" :timeout="4000" top color="info">
     ...
     <v-snackbar v-model="memberRemovedSnack" :timeout="4000" top color="info">
      <span>The member was removed from this project successfuly!</span>
    </v-snackbar>
    ...
    <v-snackbar v-model="storyEditedSnack" :timeout="4000" top color="info">
     ...
     <v-snackbar v-model="projectDeletedSnack" :timeout="4000" top color="info">
      <span>The project was deleted successfully!</span>
    </v-snackbar>
    ...
    <v-flex xs12 v-if="currentProject != undefined">
      <ProjectCard
        ...
        v-on:member-form-complete="memberAddAction"
        v-on:member-removed="memberRemovedAction"
        v-on:project-deleted="projectDeletedAction"
      ></ProjectCard>
    </v-flex>
  </v-layout>
</template>
...
export default {
  ...
  data: function() {
    return {
      ...
      memberRemovedSnack: false,
      projectDeletedSnack: false,
    };
  },
  async beforeCreate() {
  ...
  methods: {
    ...
    projectDeletedAction: async function() {
      this.projectDeletedSnack = true;
      this.getProjects();
    },
    ...
    memberAddAction: async function() {
     ...
     memberRemovedAction: async function() {
      this.memberRemovedSnack = true;
      this.getProjects();
    },
